...
void infixtoPrefix(char* infix, char* prefix) {
    ...
    int j = 0; //to traverse prefix expression
    int i = 0; //to traverse reversed infix expression
    ...
    for (i = 0; i < len; i++) {
        ...
        if (isalnum(ch)) {
            prefix[j++] = ch;
        } //all operands are simply added to the resulting prefix expression
        else if (ch == ')') {
            push(stack, ch);
        } //not open bracket because the expression is reversed
        else if (ch == '(') {
    ...
    // add remaining operators in stack to prefix expression
    ...
    // Reverse the prefix expression to get the actual prefix expression
    len = strlen(prefix);
    for (i = 0; i < len / 2; i++) {
        char temp = prefix[i];
