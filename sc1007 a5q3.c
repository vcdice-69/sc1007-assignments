...
int SD(Graph g, int v, int z) {
    ...
    int *distance = (int *)malloc(sizeof(int) * g.V);//array of distances of each vertex from v
    for (int i = 0; i < g.V; i++) {
        distance[i] = -1; // Initialize all distances to -1 (unvisited)
    }
    distance[v - 1] = 0; // Distance from v to itself is 0

    while (!isEmptyQueue(*q)) {
        int current_node = dequeue(q);
        for (int i = 0; i < g.V; i++) {
            if (g.matrix[current_node - 1][i] == 1 && distance[i] == -1) {
                distance[i] = distance[current_node - 1] + 1; // Update distance
                enqueue(q, i + 1); // Enqueue the visited node's children
                if (i + 1 == z) {
