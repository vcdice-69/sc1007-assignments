...
{
    //check if duplicate
    ListNode *check = Q1Ptr->Table[Hash(key,Q1Ptr->hSize)].head;
    while (check != NULL) {
        if (check->key ==key) return 0;
            check = check->next;
    }

    //insertion
    if (Q1Ptr->Table[Hash(key,Q1Ptr->hSize)].head == NULL){
        Q1Ptr->Table[Hash(key,Q1Ptr->hSize)].head = (ListNode*) malloc(sizeof(ListNode));
        Q1Ptr->Table[Hash(key,Q1Ptr->hSize)].head->next = NULL;
        Q1Ptr->Table[Hash(key,Q1Ptr->hSize)].head->key = key;
        Q1Ptr->Table[Hash(key,Q1Ptr->hSize)].head->pre = NULL;
        return 1;
    } //creating first node

    else {
        ListNode *temp = Q1Ptr->Table[Hash(key,Q1Ptr->hSize)].head;
        temp->pre = (ListNode*) malloc(sizeof(ListNode));
        Q1Ptr->Table[Hash(key,Q1Ptr->hSize)].head = temp->pre;
        temp->pre->pre = NULL;
        temp->pre->key = key;
        temp->pre->next = temp;
        return 1;
...
{
    ListNode *remove = HashSearch(*Q1Ptr,key);

    if (remove == NULL) return 0; //not found

    if (remove->next == NULL && remove->pre == NULL)
        Q1Ptr->Table[Hash(key,Q1Ptr->hSize)].head = NULL;
        //if only 1 item in ll, change head ptr

    else if (remove->next == NULL && remove->pre != NULL)
        remove->pre->next = NULL;
        //if last item in ll
    
    else if (remove->pre == NULL && remove->next != NULL)
        Q1Ptr->Table[Hash(key,Q1Ptr->hSize)].head = remove->next;
        //if first item in ll, change head ptr
