...
void interleaveQueue(Queue *q)
{
    Queue* temphalf1 = (Queue*)malloc(sizeof(Queue));
    Queue* temphalf2 = (Queue*)malloc(sizeof(Queue));

    int i, counter = 0, queuesize = q->ll.size;
    
    while (q->ll.head != NULL) {
        i = dequeue(q);
        if (counter < queuesize/2)
            enqueue(temphalf1,i);
        else
            enqueue(temphalf2,i);
        counter++;
    ...
    for (i = 0; i < queuesize; i++){
        if (i % 2 == 0)
            enqueue(q, dequeue(temphalf1));
        else
            enqueue(q, dequeue(temphalf2));
    }
    free(temphalf1);
    free(temphalf2);
